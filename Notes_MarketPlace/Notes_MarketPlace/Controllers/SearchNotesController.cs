...
using Notes_MarketPlace.SendMail;
...
namespace Notes_MarketPlace.Controllers
{
    public class SearchNotesController : Controller
    {
        ...
        public ActionResult Search(string search, string type, string category, string university, string course, string country, string ratings, int page = 1)
        {
            ...
            ViewBag.UniversityList = Context.SellerNotes.Where(x => x.IsActive == true && x.UniversityName != null).Select(x => x.UniversityName).Distinct();
            ViewBag.CourseList = Context.SellerNotes.Where(x => x.IsActive == true && x.Course != null).Select(x => x.Course).Distinct();
            ...
            // get published notes
            var noteslist = Context.SellerNotes.Where(x => x.Status == 11);
            ...
            // create list object of search notes model
            List<SearchNotesModel> searchnoteslist = new List<SearchNotesModel>();
            ...
            if (String.IsNullOrEmpty(ratings))
            {
                foreach (var item in noteslist)
                {
                    ...
                    SearchNotesModel note = new SearchNotesModel()
                    {
                        Note = item,
            ...
            IEnumerable<SearchNotesModel> result = searchnoteslist.AsEnumerable().Skip((page - 1) * 9).Take(9);
            // total result count
        ...
        public ActionResult Notes(int id)
        {
            ...
            IEnumerable<ReviewsViewModel> reviews = from review in Context.SellerNotesReviews
                                                    ...
                                                    join userprofile in Context.UserProfiles on review.ReviewedByID equals userprofile.User_ID
            ...
            NotesDetailModel notesdetail = new NotesDetailModel();
                ...
                using (var memoryStream = new MemoryStream())
                {
                    using (var ziparchive = new ZipArchive(memoryStream, ZipArchiveMode.Create, true))
                    {
                        foreach (var item in dir.GetFiles())
                        {
                            ...
                            ziparchive.CreateEntryFromFile(filepath, item.ToString(), CompressionLevel.Optimal);
                        }
                    }
                    // return zip
