...
namespace Notes_MarketPlace.Controllers
{
    public class BuyerRequestController : Controller
    {
        ...
        public ActionResult BuyerRequest(string search, string sort, int page = 1)
        {
            ...
            // get buyer requests
            IEnumerable<BuyerRequestModel> buyerrequest = from download in Context.Downloads
                                                          join users in Context.Users on download.Downloader equals users.ID
                                                          join userprofile in Context.UserProfiles on download.Downloader equals userprofile.User_ID
                                                          ...
                                                          select new BuyerRequestModel
                                                          {
                                                              ...
                                                              Buyer = users.EmailID,
                                                              PhoneNo = userprofile.Phone_number___Country_Code + " " + userprofile.Phone_number,
        ...
        private IEnumerable<BuyerRequestModel> SortTableBuyerRequest(string sort, IEnumerable<BuyerRequestModel> table)
        {
            switch (sort)
            {
                case "Title_Asc":
        ...
        public ActionResult AllowDownload(int id)
        {
            ...
            Download download = Context.Downloads.Find(id);
            // check if logged in user and note seller is same or not
            if (user.ID == download.Seller)
            {
                // get sellernoteattachement object
                SellerNotesAttachement attachement = Context.SellerNotesAttachements.Where(x => x.NoteID == download.NoteID && x.IsActive == true).FirstOrDefault();
